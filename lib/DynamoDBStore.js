...
export default class DynamoDBStore extends Store {
  ...
  setOptionsAsInstanceAttributes(options: Object): void {
    ...
    const {
      ...
      useExistingTable = DEFAULT_USE_EXISTING_TABLE,
      rangeKey = DEFAULT_RANGE_KEY,
      rangeVal = DEFAULT_RANGE_VALUE,
    } = table;
  ...
  set(sid: string, sess: Object, callback: Function): void {
    try {
      ...
      if(this.rangeKey){
        params.Item[this.rangeKey] = this.rangeVal
      }
  ...
  async get(sid: string, callback: Function): Promise<void> {
    try {
      ...
      if(this.rangeKey){
        params.Key[this.rangeKey] = this.rangeVal
      }
